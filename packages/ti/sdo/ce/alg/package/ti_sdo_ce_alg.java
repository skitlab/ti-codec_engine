...
public class ti_sdo_ce_alg
{
    ...
    void Algorithm$$OBJECTS()
    {
        Proto.Obj po, spo;
        Value.Obj vo;

        po = (Proto.Obj)om.bind("ti.sdo.ce.alg.Algorithm.Module", new Proto.Obj());
        vo = (Value.Obj)om.bind("ti.sdo.ce.alg.Algorithm", new Value.Obj("ti.sdo.ce.alg.Algorithm", po));
        pkgV.bind("Algorithm", vo);
        // decls 
    }

    void Algorithm$$CONSTS()
    {
        // module Algorithm
        om.bind("ti.sdo.ce.alg.Algorithm.MAXGROUPID", 20L);
    }

    void Algorithm$$CREATES()
    {
        Proto.Fxn fxn;
        StringBuilder sb;
    ...
    void Algorithm$$FUNCTIONS()
    {
        Proto.Fxn fxn;

    }

    void Algorithm$$SIZES()
    {
    }

    void Algorithm$$TYPES()
    {
        Scriptable cap;
        Proto.Obj po;
        Proto.Str ps;
        ...
        po = (Proto.Obj)om.findStrict("ti.sdo.ce.alg.Algorithm.Module", "ti.sdo.ce.alg");
        po.init("ti.sdo.ce.alg.Algorithm.Module", $$Module);
    ...
    void $$SINGLETONS()
    {
        ...
        sb.append("pkg.build.libraries = [\n");
            sb.append("'lib/debug/Algorithm_BIOS.aem3',\n");
            sb.append("'lib/release/Algorithm_BIOS.aem3',\n");
            sb.append("'lib/debug/Algorithm_BIOS.ae674',\n");
            sb.append("'lib/release/Algorithm_BIOS.ae674',\n");
        ...
        sb.append("pkg.build.libDesc = [\n");
            sb.append("['lib/debug/Algorithm_BIOS.aem3', {target: 'ti.targets.arm.elf.M3', suffix: 'em3'}],\n");
            sb.append("['lib/release/Algorithm_BIOS.aem3', {target: 'ti.targets.arm.elf.M3', suffix: 'em3'}],\n");
            sb.append("['lib/debug/Algorithm_BIOS.ae674', {target: 'ti.targets.elf.C674', suffix: 'e674'}],\n");
            sb.append("['lib/release/Algorithm_BIOS.ae674', {target: 'ti.targets.elf.C674', suffix: 'e674'}],\n");
            sb.append("['lib/debug/Algorithm_noOS.av5T', {target: 'gnu.targets.arm.GCArmv5T', suffix: 'v5T'}],\n");
            sb.append("['lib/debug/alg.av5T', {target: 'gnu.targets.arm.GCArmv5T', suffix: 'v5T'}],\n");
            sb.append("['lib/release/Algorithm_noOS.av5T', {target: 'gnu.targets.arm.GCArmv5T', suffix: 'v5T'}],\n");
            sb.append("['lib/release/alg.av5T', {target: 'gnu.targets.arm.GCArmv5T', suffix: 'v5T'}],\n");
        sb.append("];\n");
        Global.eval(sb.toString());
    }

    void Algorithm$$SINGLETONS()
    {
        Proto.Obj po;
        Value.Obj vo;

        vo = (Value.Obj)om.findStrict("ti.sdo.ce.alg.Algorithm", "ti.sdo.ce.alg");
        po = (Proto.Obj)om.findStrict("ti.sdo.ce.alg.Algorithm.Module", "ti.sdo.ce.alg");
        vo.init2(po, "ti.sdo.ce.alg.Algorithm", $$DEFAULT, false);
        vo.bind("Module", po);
        ...
        vo.bind("$$meta_iobj", om.has("ti.sdo.ce.alg.Algorithm$$instance$static$init", null) ? 1 : 0);
        vo.bind("$$fxntab", Global.newArray());
        vo.bind("$$logEvtCfgs", Global.newArray());
        vo.bind("$$errorDescCfgs", Global.newArray());
        vo.bind("$$assertDescCfgs", Global.newArray());
        Value.Map atmap = (Value.Map)vo.getv("$attr");
        atmap.setElem("", "./Algorithm.xdt");
        atmap.seal("length");
        vo.bind("TEMPLATE$", "./Algorithm.xdt");
        pkgV.bind("Algorithm", vo);
        ((Value.Arr)pkgV.getv("$unitNames")).add("Algorithm");
    }
    ...
    void $$INITIALIZATION()
    {
        ...
        Global.callFxn("module$meta$init", (Scriptable)om.findStrict("ti.sdo.ce.alg.Algorithm", "ti.sdo.ce.alg"));
        Global.callFxn("init", pkgV);
        ((Value.Obj)om.getv("ti.sdo.ce.alg.Algorithm")).bless();
